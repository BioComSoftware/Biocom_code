...
from common.checks import (checkObject, fileExists, fullPathExists, checklist)
...
#     def __new__(cls, callobj, *args, **kwargs):
    def __new__(cls, *args, **kwargs):
        ...
        try:
            ...
            log.debug(''.join(['ConfigHandler.__new__: ', 
                               'Calling script is not a class object. ',
                               'Falsifying.']))
            callobj = FalseSelf()
    ...
    def __init__(self, *args, **kwargs):
        """"""
        log.debug('Starting confighandler.ConfigHandler.__init__()') #333
        # This grabs the calling object, so var can be loaded into it

        log.debug('Geting self.callobj') #333
        try:
            self.callobj = inspect.stack()[1][0].f_locals['self']
            log.debug('self.callobj=' + str(self.callobj)) #333
        except KeyError as e:
            log.debug(''.join(['ConfigHandler.__init__: ', 
                               'Calling script is not a class object. ',
                               'Falsifying.']))          
            self.callobj = FalseSelf()
        ...
        log.debug('self.caller_name=' + str(self.caller_name)) #333

        # Singleton. If confighandler object exists, do not re-run __init__,
        # just return
        if self.__exists:
            log.debug('ConfigHandler.__exists == True') #333
        ...
        log.debug('ConfigHandler._set_parameters(dict(kwargs))')
        self._set_parameters(dict(kwargs))
        log.debug('(COMPLETE)ConfigHandler._set_parameters(dict(kwargs))')
...
#             return self._getSyslog()
    ...
    def _load_all_vars(self, kwargs):
        """"""
        # If file error or empty config file, return 
        if not self.open_file(): return False
...
#         self._set_mandatory_defaults(self.MANDATORY_DEFAUTS)
    ...
    def _messages(self, key, *args, **kwargs):
        if (
            (re.match('.*no.*header.*', str(key).lower())) or
            (re.match('.*no.*param.*', str(key).lower()))
            ):
            return (''.join(["Unable to find parameters in config_file. ", 
                     "Configuration file may be empty. ", 
                     "At least one [HEADER] and one 'parameter = value' ", 
                     "is required for configuration file function. ", 
                     "Continuing with NULL ConfigHandler object. "
                     ]))
...
#                                                 "'."]))
        ...
        if not _found: 
#             raise AttributeError(''.join(["Unable to find variable '",
            msg = self._messages('noparam')
            msg = (''.join([msg, ' ', 
...
if __name__ == "__main__":
    from qrnote.loghandler import log
