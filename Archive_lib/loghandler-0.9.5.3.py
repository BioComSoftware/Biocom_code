...
class SetLogger(object):
    ...
    def __init__(self,
        ...
        if self.__exists:
            ...
            app_name = self._check_app_name(app_name)
            if (
                (app_name != None) and 
                ('none' not in str(app_name).lower()) and 
                (app_name != self.app_name)
                ):
                change_list.append({'app_name':app_name})
                
            log_level    = self._check_log_level(log_level)    
            if ((log_level != None) and (log_level != self.log_level)):
                change_list.append({'log_level':log_level})

            screendump   = self._check_screendump(screendump)
...
#         self.create_time = datetime.datetime.now().time() #333 FOR DEBUGGING
        
        self.app_name_default     = 'loghandler',
        self.logfile_default      = 'syslog'
...
#         self._remove_logger(app_name)
    ...
    def _change_logfile(
        ...
        """
        self.logger.debug("loghandler._change_logfile()")
...
#         else:
    ...
    def _check_create_paths(self, create_paths):
        if (
            (type(create_paths) is not bool) or 
            (create_paths is None)
            ):
            try:
    ...
    def _check_formatter(self, formatter = None):
...
#             raise TypeError(e)
    ...
    def _check_logfile_format(self, logfile = None):
        ...
        if (
            ('syslog' in logfile.lower()) or 
            ('system' in logfile.lower()) 
            ):
#             return self._getSyslog()
        ...
        if not fullPathExists(logfile, create = self.create_paths):
            err = ''.join([
...
#         #Level
    ...
    def _check_screendump(self, screendump):
        if (
            (type(screendump) is not bool) or 
            (screendump is None)
            ):
            try:
                return self.screendump
...
#         if re.match("^(\s){0,}(<){0,1}stderr(>){0,1}(\s){0,}$", str(path).lower()):
        ...
        for handler in self.logger.handlers:
            try:
                if handler.__dict__['stream'].name == path:
                    return True
...
#         self.logfile  = os.path.basename(dest)
    ...
    def _remove_handler(self, handle = None):
        """"""
#         print 'str(handle).lower()) = ', str(handle).lower() #333
#         if re.match("^(\s){0,}(<){0,1}stderr(>){0,1}(\s){0,}$", str(handle).lower()):
#             print 'Match stderr' #333
#             handle = "<stderr>"
# 
...
#         for handler in self.logger.handlers:
#             print "_isExistingHandler:: handler.__dict__['stream'].name='" +str(handler.__dict__['stream'].name) +"'" #333  
#             if handler.__dict__['stream'].name == handle:
#                 print 'THIS IS IT!!'
...
#             print "logging.Logger.manager.loggerDict["+str(app_name) + "] = ", logging.Logger.manager.loggerDict[app_name] #333 
#             for h in list(logging.Logger.manager.loggerDict[app_name].handlers):
#                 print 'removing handler: "' + str(h) + '"' #333 
...
#             del logging.Logger.manager.loggerDict[app_name]
    ...
    def _set_filehandler(
        ...
        if ('syslog' in self.logfile):

            if (sys.platform == "darwin"):
                # Apple made 10.5 more secure by disabling network syslog:
                fh = logging.handlers.SysLogHandler("/var/run/syslog", 
                                                    facility='user')
#                 handler_address = "/var/run/syslog"

            elif sys.platform == "linux2":
                fh = logging.handlers.SysLogHandler("/dev/log", 
                                                    facility='user')
#                 handler_address = "/dev/log"
# 
...
#                 pass                
                ...
                # Is this just 'other' OSX, or will it work on all os
                fh = logging.handlers.SysLogHandler(('localhost', 514), 
                                                    facility='user')
#                 handler_address = ('localhost', 514)

#             fh = logging.handlers.SysLogHandler(handler_address, 
#                                                 facility='user')
...
if __name__ == '__main__':
    # Set logger manually
    log.info('Starting first call of loghandler...',
             app_name   = 'loghandlertest1',
             logfile    = './logfiletest1',
             log_level  = 10,
             screendump = True, 
             create_paths = True
             )       
    ...
    log.info('Starting second call of loghandler...',
