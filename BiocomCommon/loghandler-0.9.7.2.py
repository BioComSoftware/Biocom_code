...
class log(object):
    ...
    def _setLogger(self, *args, **kwargs):
        ...
        app_name     = kwargs.pop('app_name',        'UNCHANGED')
        logfile      = kwargs.pop('logfile',         'UNCHANGED')
        log_level    = kwargs.pop('log_level',       'UNCHANGED')
        screendump   = kwargs.pop('screendump',      'UNCHANGED')
        format       = kwargs.pop('format',          'UNCHANGED')
        create_paths = kwargs.pop('create_paths',    'UNCHANGED')
        migrate      = kwargs.pop('migrate',         'UNCHANGED')
...
#         logfile = self._check_logfile(logfile) # returns full path
        ...
        if (self.logfile == 'syslog'):
            ...
            if (sys.platform == "darwin"): fh = logging.handlers.SysLogHandler("/var/run/syslog", facility='user')
            elif sys.platform == "linux2": fh = logging.handlers.SysLogHandler("/dev/log",  facility='user')
            # Is this just 'other' OSX, or will it work on all os
            else: fh = logging.handlers.SysLogHandler(('localhost', 514), facility='user')
#             fh = logging.handlers.SysLogHandler(handler_address, facility='user')
