...
class errorLogger(object):
    ...
    def transfer(self, full_log_path):
        """
        :NAME:
            errorLogger.transfer()
        
        :DESCRIPTION:
            transfer(full_log_path)
            
            Transfers the contents of the existing temporary log file to the 
            full_log_path location specific. 
            
            NOTE: The full_log_path MUST be the full path including name 
                  extensions. There is no path or name  checking in this class.
                  
        :ARGS:
            full_log_path: The FULL PATH to the new logfile location.
                     I.e. '/var/log/syslog'
                          'home/user/mylogs/logfile.log' 

            NOTE: The full_log_path MUST be the full path including name 
                  extensions. There is no path or name  checking in this class.

        :RETURNS:
            True/error_message for success or failure. 
        """
        self.write(''.join(["errorlogger.errorLogger.transfer: ", 
                            "transferring errorlogger temporary log file '", 
                            str(self.logfile), 
                            "' to '", 
                            str(full_log_path), 
                            "'."]))
        try:
            ...
            e = ''.join(["errorLogger.transfer: full_log_path is not a string.", 
                        str(e)])
            return str(e)
               
        if not checkPathFormat(full_log_path, endslash = False):
            e = ''.join(["errorLogger.transfer: Parameter full_log_path (", 
                         str(full_log_path), 
                         " does not appear to be a correctly formed path."])
            return str(e)
        
        #Read in the current temp log file contents
        try:
            _file_contents = self._read()
        except Exception, e:
            e = ''.join(["errorLogger.transfer:", 
                         "Failure attempting to read existing errorlog log. ", 
                         str(e)])
            return e
        ...
        try:    
            with open(full_log_path, _mode) as FH:
                for line in _file_contents:
                    print >>FH, line
        except Exception, e:
            e = ''.join(["errorLogger.transfer:", 
                         "Failure opening and/or writing new data to: '", 
                         str(full_log_path),
                         "'. ", 
                         str(e)])
            return e
...
if __name__ == "__main__":
    ...
    o = errorLogger('errorLogger', screendump=True, debug = False)
    print "Done."
    ...
    print; print "errorLogger.transfer()..."
    result = o.transfer("./transferred.log")
    print "result: ", l
    print "Done." 
